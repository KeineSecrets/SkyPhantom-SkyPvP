...
public class StatsCommand extends Command {
    ...
    public StatsCommand() {
        super("stats", "", "", Collections.singletonList("statistik"));
    }
    ...
    public boolean execute(CommandSender commandSender, String s, String[] args) {
        if (commandSender instanceof Player) {
            ...
            if (args.length < 1) {
                ...
                player.sendMessage("§8▎ §7Tokens§8: §6" + new DecimalFormat("#,###.##").format(statsProvider.getCoins()).replace(",", ".").replace(".", "§8'§6") + "§8 ▪ §7Votes§8: §90 §8▪ §7Spielzeit§8: §600h 00m");
                player.sendMessage("");
                return true;
            }
            ...
            if (target != null && target.hasPlayedBefore()) {
                ...
                player.sendMessage("");
                player.sendMessage("§8▎ §7Name§8: §a" + target.getName() + "§8 ▪ §7Status§8: " + (target.isOnline() ? "§aonline" : "§coffline"));
